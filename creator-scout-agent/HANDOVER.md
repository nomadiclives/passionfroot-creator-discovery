...
| **Scheduler under a real clock** | `--once` and the next-run arithmetic are tested; the blocking loop has not been left running across an actual scheduled fire. On Render's free tier it never will — `render.yaml` defines a single web service, the app starts no timer of its own, and a free instance sleeps when idle. That needs a worker or a cron job on a paid plan. **The screen now says this** rather than showing a next-run date nobody is keeping: `Run now` is marked as the only control that starts a run, and the next-run figure is tagged *if run by hand*. |
